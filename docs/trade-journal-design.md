...
The `PRE_TRADE` image is never overwritten after the trade closes — a `READY` row is immutable historical evidence, enforced by two `@@unique` database constraints (`[setupId, type, chartConfigVersion]` and `[tradeId, tradeSource, type, chartConfigVersion]`), not just convention. A rendering-behavior change bumps `chartConfigVersion` and creates a new row rather than mutating an old one.
