...
**Post-trade** (recorded later, separately): `JournalTrade`'s actual-entry/actual-exit/P&L/MFE/MAE/`outcome` fields (set once, at `recordJournalTradeEntry`/`closeJournalTrade` time), and `PostTradeAnalysis`. MFE/MAE and `outcome` are always server-computed - see "Journal trades" below.
...
An append-only event log, not a replacement for the relational tables above: `packages/database`'s `journal-events.ts` repository exposes only `createJournalEvent`/`listJournalEvents`/`getSetupTimeline`, never an update or delete. The event vocabulary (`JournalEventType`): `SETUP_CREATED`, `STRATEGY_EVALUATED`, `RISK_CALCULATED`, `SETUP_APPROVED`/`SETUP_REJECTED`/`SETUP_INVALIDATED`/`SETUP_EXPIRED`, `TRADE_READY`, `TRADE_EXECUTED`, `TRADE_SKIPPED`, `TRADE_CLOSED`, `POST_TRADE_ANALYSIS_CREATED`, `STRATEGY_VERSION_PROPOSED` (Milestone 2), plus `WEBHOOK_RECEIVED`, `WEBHOOK_NORMALIZED`, `SIGNAL_ACCEPTED`, `WEBHOOK_DUPLICATE_DETECTED`, `WEBHOOK_REJECTED`, `WEBHOOK_PROCESSING_FAILED` (Milestone 3; see "TradingView webhook ingestion" below), plus `NOTIFICATION_QUEUED`/`NOTIFICATION_SENDING`/`NOTIFICATION_SENT`/`NOTIFICATION_RETRYING`/`NOTIFICATION_FAILED` against a new `NOTIFICATION_DELIVERY` entity type (Milestone 6; emitted by `packages/database`'s `notification-deliveries.ts` repository alongside every `NotificationDelivery` status transition - see `docs/notifications.md`). Later milestones add `AGENT_STARTED`/`AGENT_COMPLETED`/`AGENT_FAILED`, `RESEARCH_HYPOTHESIS_CREATED`, `STRATEGY_VERSION_APPROVED`, `STRATEGY_PAUSED`, etc. via a small additive migration once the runtime agents that emit them exist; they are deliberately not pre-added now.
...
**MFE/MAE and `outcome` are computed server-side, never client-supplied (Milestone 6).** `closeJournalTradeSchema` accepts no `mfe`/`mae` fields at all - an earlier version of this API did, and that input surface was removed in Milestone 6. `packages/database`'s `closeJournalTrade` (`packages/database/src/repositories/journal-trades.ts`) computes both itself:

- `outcome` (`WIN` / `LOSS` / `BREAKEVEN`) is derived purely from `netPnl`'s sign inside `computeJournalTradeClose`: positive is `WIN`, negative is `LOSS`, exactly zero is `BREAKEVEN`. It is never client-supplied and never computed anywhere outside this one function.
- `mfe`/`mae` (maximum favorable/adverse excursion) are computed by calling `packages/risk-engine`'s `calculateExcursions(candles, entryPrice, direction)` over the real candle series between the trade's `entryTimestamp` and `exitTimestamp`, mirroring `packages/backtester`'s `walkTradeForward` formula exactly so a manually-closed trade's MFE/MAE is computed identically to a backtested trade's. This is only possible when the `JournalTrade` has a `Setup` lineage (`setupId` is set): a `Setup`'s `MarketSnapshot` supplies the `timeframe` needed to know which candle series to query. **A `JournalTrade` with no `Setup` lineage (`setupId: null`) - a manually logged trade with no originating setup - legitimately gets `mfe: null`/`mae: null`.** "Unknown stays unknown" rather than a fabricated value or a thrown error blocking the close; the same rule applies if the candle query for a setup-linked trade happens to return zero candles.

## Skip workflow

A `READY` setup a human deliberately does not take is recorded via `POST /setups/:id/skip` (`SetupService.skip`), which creates a `JournalTrade` directly at `executionMode: "SKIPPED"` (see "Journal trades" above for why this bypasses `PLANNED`). An optional `reason` may be attached, drawn from the `SkipReason` enum (`packages/database/prisma/schema.prisma`):
...
`reason` is always optional (`skipSetupSchema` in `packages/shared-types/src/notifications.ts` defines it as `z.enum(SKIP_REASONS).optional()`) and is never auto-assigned - only a human choosing SKIP TRADE on the dashboard sets it. Skipping is a trade decision, not a `Setup` lifecycle change: `skip()` never transitions the `Setup`'s own `status`, which can still separately expire/invalidate on its own terms via `updateStatus`.
