...
- **Retry policy.** Both the TradingView webhook queue and the setup-expiration queue are configured with `attempts: 3` and exponential backoff. For the TradingView webhook queue this policy lives in one shared constant, `TRADINGVIEW_WEBHOOK_JOB_OPTIONS` (`packages/shared-types/src/tradingview.ts`), passed as `defaultJobOptions` at **both** `BullModule.registerQueue` call sites for that queue name — `apps/api/src/webhooks/tradingview-webhook.module.ts` (the original ingest enqueue) and `apps/worker/src/app.module.ts` (the reconciliation sweep's re-enqueue). This matters because `defaultJobOptions` is per-Queue-*instance*, not per-queue-name: each app's `registerQueue` call creates its own instance, so a policy set only on one side would leave jobs added through the other side's instance with BullMQ's bare defaults (1 attempt, no backoff). The setup-expiration queue's policy is configured directly in `apps/worker/src/app.module.ts`, the only app that ever enqueues onto it. So a transient failure (a momentary Postgres or Redis blip) is retried automatically rather than marking the event `FAILED` permanently on the first attempt, from either enqueue path. Both processors are written to be safe under retry: `ALREADY_RESOLVED_STATUSES` in `tradingview-webhook.processor.ts` skips reprocessing an event that already reached a terminal outcome, `Setup.sourceWebhookEventId`'s uniqueness (see "Idempotency" above) prevents a retry that lands *between* `Setup` creation and the event being marked `PROCESSED` from creating a second `Setup`, and the terminal-status check in `setup-expiration.processor.ts` prevents a duplicate expiration-job schedule from ever overwriting an already-resolved `Setup`.
- **A narrow crash window between event persistence and enqueue (fixed).** In `tradingview-webhook.service.ts`, the `InboundWebhookEvent` row is created (claiming its fingerprint) before the BullMQ job is enqueued. If the process crashes in that exact window, the row exists but no job was ever queued to process it, and TradingView's retried delivery (same fingerprint) is now recorded as a duplicate rather than actually queued, since the fingerprint is already claimed. This is a narrow, low-likelihood window and never risked a duplicate `Setup`, but it could leave an event stuck at `RECEIVED`/`QUEUED` with nothing left to process it. `apps/worker/src/webhook-reconciliation/webhook-reconciliation.processor.ts` now closes this: a BullMQ repeatable job (registered via `Queue.upsertJobScheduler` with a fixed scheduler id, so re-registering it on every worker restart never creates a duplicate schedule) periodically calls `findStaleInboundWebhookEvents` (`packages/database/src/repositories/inbound-webhook-events.ts`) for any `InboundWebhookEvent` still `RECEIVED`/`QUEUED` past `WEBHOOK_RECONCILIATION_STALE_THRESHOLD_MINUTES` and reconciles its processing job.

  Reconciliation is **job-state-aware**, not a blind re-`add()`. BullMQ's jobId-based dedup (both this re-enqueue and the original enqueue in `tradingview-webhook.service.ts` pass the same `jobId: event.id`) only prevents a duplicate `add()` while a job with that id is still retained under a state BullMQ keeps a hash for — and since neither queue sets `removeOnComplete`/`removeOnFail`, that includes `failed` and `completed`, which are retained forever, not just `waiting`/`active`/`delayed`. A blind `add()` would therefore permanently no-op once a job reached `failed` (for example, its `attempts` exhausted during a Postgres outage): the event would stay `RECEIVED`/`QUEUED` forever, since every later sweep's `add()` would just silently return the existing failed job. So before adding anything, the processor checks the job's actual state via `queue.getJob(event.id)`:
  - no job found → `add()` (the original safe case, unchanged).
  - `failed` → `job.retry("failed")`, an atomic BullMQ operation that only moves a job genuinely still in the failed set back to waiting — this introduces no new race.
  - `completed` → logged as a warning and skipped. A completed job implies the event's `processingStatus` should already have advanced past `RECEIVED`/`QUEUED`; seeing this combination means something else is wrong, so it's surfaced loudly rather than silently worked around.
  - `waiting`/`active`/`delayed` → skipped. A job for this event is already going to run (or is running) — this is what actually prevents two workers from concurrently racing to read this row's `processingStatus` before either writes `PROCESSING`.

  `Setup.sourceWebhookEventId`'s DB unique constraint (paired with `TradingViewWebhookProcessor`'s `ALREADY_RESOLVED_STATUSES` check) remains as defense-in-depth protecting `Setup` creation specifically, on top of the job-state guarantee above.
- **`getFullTradingViewTimeline`'s chronological merge has a tiebreaker for two events sharing an identical millisecond timestamp (fixed).** `JournalEvent.sequence` (`@default(autoincrement())`) is a monotonic insertion-order column, populated by Postgres itself, independent of `timestamp`'s millisecond precision. `mergeJournalEventRows` (`packages/database/src/repositories/journal-events.ts`) sorts by `(timestamp, sequence)`, never relying on `Array.prototype.sort`'s incidental stability, and every timeline query (`listJournalEvents`, `getSetupTimeline`, `getInboundWebhookEventTimeline`, `getFullTradingViewTimeline`) shares the same `orderBy`.
